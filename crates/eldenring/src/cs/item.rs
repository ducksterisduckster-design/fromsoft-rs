...
pub enum ItemGrantError {
    /// The executable RVA could not be converted to a virtual address.
    #[error("{0}")]
    Lookup(String),
...
pub fn map_item_man_grant_item_va() -> Result<u64, ItemGrantError> {
    Program::current()
        ...
        .map_err(|err| {
            ItemGrantError::Lookup(format!(
                "failed to convert ER ItemGive RVA {:#x}: {err:?}",
                rva::get().map_item_man_grant_item
            ))
        })
}
