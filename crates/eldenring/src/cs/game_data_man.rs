...
use pelite::pe64::Pe;
use shared::{FromStatic, OwnedPtr, Program, load_static_indirect};
...
impl GameDataMan {
    ...
    pub fn give_item_directly(&mut self, item: ItemId, quantity: u32) {
        let quantity = i32::try_from(quantity)
            .expect("quantity for lua_event_man_give_item_directly should fit i32");

        self.edit_item_directly(item, quantity);
    }
    ...
    fn edit_item_directly(&mut self, item: ItemId, quantity: i32) {
        let va = Program::current()
            .rva_to_va(crate::rva::get().lua_event_man_give_item_directly)
            .expect("Call target for lua_event_man_give_item_directly was not in exe");
        let edit_item_directly: extern "C" fn(u32, u32, i32) = unsafe { std::mem::transmute(va) };

        // ER's direct helper treats positive quantities as adds and negative
        // quantities as removals. Keep the public API shaped like DS3 by
        // exposing separate add/remove methods.
        edit_item_directly((item.category() as u32) << 28, item.param_id(), quantity);
    }
